...
const lines = [
  ["Immensity", "Ka", "East Meets West"],
  ["Pado", "Eve", "Recupera Se", "Follow Me Through it All"],
  ["Unearthed", "Avarice", "Eaclipse", "Reverie", "Balance"],
  ["Self Love", "Celestial Vanity", "Shattered Silk", "Aporia"],
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (hasSeenPreloader) setIsPreloaderComplete(true);
  }, []);

  useEffect(() => {
    const handleKeyDown = (event: { key: string }) => {
      if (event.key === "ArrowRight") setActIndex((prev) => (prev === 4 ? 1 : prev + 1));
      if (event.key === "ArrowLeft") setActIndex((prev) => (prev === 1 ? 4 : prev - 1));
    };
        ...
        >
          <div className="relative flex items-center">
            <span className={`absolute left-0 transform translate-y-[-20px] uppercase ${greyMonoTrial.className}`}>
              {romanNumerals[prevIndex(actIndex)]}. {acts[prevIndex(actIndex)]}
            </span>
            <div className="h-[0.5px] w-[15vw] md:w-[20vw] bg-gray-600" />
          </div>
          <div
            className="h-[1px] w-[4vw] md:w-[5vw] bg-white absolute transition-all duration-300"
            style={{
              transform: isLeftHovered ? "translateX(0%)" : "translateX(290%)",
            }}
          />
          {/* MIDDLE LINE */}
          <div className="absolute left-1/2 transform -translate-x-1/2 w-full flex justify-center">
            <div className="h-[0.5px] w-[62vw] md:w-[55vw] bg-gray-600" />
          </div>
        </button>

        {/* CENTER TICKS */}
<div className="absolute left-1/2 transform -translate-x-1/2 translate-y-[-50%] flex justify-center w-full pointer-events-none">
  <div
    className="flex justify-around px-2 md:px-4 pointer-events-auto"
    style={{
      width: `${Math.min(80, lines[actIndex - 1].length * 10)}vw`,
    }}
  >
    {lines[actIndex - 1].map((item, index) => {
      const globalLineNum = lines
        .slice(0, actIndex - 1)
        .reduce((acc, act) => acc + act.length, 0) + index + 1;

      return (
        <Link
  key={index}
  href={`/lines/${globalLineNum}`}
  className="relative flex flex-col items-center cursor-pointer px-4"
  onMouseEnter={() => setHoveredTick(index)}
  onMouseLeave={() => setHoveredTick(null)}
>
  <p
    className={`absolute bottom-full mb-2 text-sm whitespace-nowrap transition-colors duration-200 ${greyMonoTrial.className} ${
      hoveredTick === index ? "text-white" : "text-gray-400"
    }`}
  >
    {item}
  </p>
  <div
    className={`h-[18px] w-[1px] ${
      hoveredTick === index ? "bg-white" : "bg-gray-400"
    }`}
    style={{
        ...
        >
          <div className="relative flex items-center">
            <span className={`absolute right-0 transform translate-y-[-20px] uppercase ${greyMonoTrial.className}`}>
              {romanNumerals[nextIndex(actIndex)]}. {acts[nextIndex(actIndex)]}
            </span>
            <div className="h-[0.5px] w-[15vw] md:w-[20vw] bg-gray-600" />
          </div>
          <div
