...
vec3 curl( in vec3 p, in float noiseTime, in float persistence ) {
    ...
    for (int i = 0; i < 2; ++i) {
        // change twoPowI & scale to alter noise
        // scale alters how 'wavy' the noise is
        float twoPowI = pow(8.0, float(i));
...
void main()	{
    ...
    // Circular force controls speed
	float circlularForce = 0.5 - smoothstep(0.3,1.4, abs(pos.x-radius));
    // change 1.0 to something like 2.0 to go FAST
	float angle = atan(pos.y, pos.x) - info.y*0.3*mix(0.5,1.,circlularForce);

    // Change the radius (originally info.x, 1.8), (info.x, 0.5)
    // Change 0.0 in "0.0*sin(angle*2. + time*0.0)" to .8 to pinch the center inside a bit
    // Change time to alter rotation speed
    // Set angle zero, time zero for perf circle

    // ---------- HUBRIS V1 ----------------------------
	// float targetRadius = mix(
	// 	info.x,2.0, 
	// 	0.8 + .45*sin(angle*1. + time*0.)
	// );

    // ---------- HUBRIS V2 ----------------------------
    // float targetRadius = mix(
	//	info.x,3.0, 
	//	0.25 + .8*sin(angle*1.0 + time*0.1)
	//);
    // ---------- HUBRIS V3 ----------------------------
    float targetRadius = mix(
		info.x,2.0, 
		0.25 + 1.8*sin(angle*2.0 + time*0.1)
	);
